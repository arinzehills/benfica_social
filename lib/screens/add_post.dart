...
import 'package:benfica_social/widgets/network_image_display.dart';
...
class _AddPostState extends State<AddPost> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          elevation: 0,
          title: Text(
            widget.post != null ? "Edit" : 'Add Post',
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                // _showModalBottomSheet(context, user.id, widget.post!.post_id);
              },
            ),
          ],
        ),
                  ...
                  child: Column(
                      ...
                      children: <Widget>[
                        Padding(
                          ...
                          child: MyTextField(
                            hintText: widget.post?.title ?? 'Enter title',
                            initiaiValue:
                                widget.post == null ? null : widget.post!.title,
                            validator: (val) =>
                                val!.isEmpty ? 'Enter a title' : null,
                            onChanged: (v) => {
                              setState(() {title = v; valueChanged = true;})
                            },
                          ),
                        ),
                        Padding(
                          ...
                          child: Container(
                            child: TextFormField(
                              ...
                              onChanged: (val) {
                                                             setState(() {description = val; valueChanged = true;});

                              },
                            ),
                          ),
                        ),
                        if (widget.post != null && images!.isEmpty)
                          NetworkImageDisplay(
                            images: widget.post!.imageUrls,
                          ),
                        if (images!.isNotEmpty) ImageDisplay(images: images!),
                        // (images!.length == 0&&widget.post==null)
                        //     ? SizedBox()
  ...
  Future selectImages() async {
    ...
    setState(() {
      images = paths.map((path) => File(path)).toList();
valueChanged=true;
      // Get the names of the images
      // imagenames = images.map((image) => Path.basename(image.path)).toList();
    });

    // Print the names of the selected images
    // print('Selected image names: $imagenames');
  }

  Future<void> addPost() async {
    try {
      setState(() => loading = true);
      var res = await PostService()
          .addPost(title: title, description: description, images: images);
      var response = json.decode(res.body);
      print(response);
      if (response['success'] == true) {
        Constants.showTaost(msg: 'Post added successfully');
        Navigator.pop(context);
        // MyNavigate.navigatepushuntil(HomePageNavigation(), context);
      } else {
  ...
  Future<void> editPost(title, description) async {
    if(!valueChanged) return;
    try {
      setState(() => loading = true);
      var res = await PostService().updatePost(
          id: widget.post!.id,
          title: title,
      ...
      if (response['success'] == true) {
        Constants.showTaost(msg: 'Post Edited successfully');
        Navigator.pop(context);
        Navigator.pop(context);
        // MyNavigate.navigatepushuntil(HomePageNavigation(), context);
      } else {
